...
const TaskAdmin = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // preload users (leaders + members) for right panel + left assignee chips
        const uniqMemberIds = [
          ...new Set(
            arr.flatMap(p => [p.leader_id, ...(p.member_ids || [])]).filter(Boolean)
          ),
        ];
        if (uniqMemberIds.length) {
          ...
          if (ru.ok && Array.isArray(users)) {
            ...
            // leaders map is a subset for quick lookups
            const leaderMap = {};
            arr.forEach(p => {
  ...
  // members for selector (leaders + members from ALL projects)
...
// put this inside your component (near other hooks)
  ...
  return (
    ...
    <div className="p-6 font-sans bg-white min-h-screen w-full flex">
      ...
      <div className="w-1/3 pl-6 border-l border-gray-200">
        {/* Member header with selector */}
        {/* MEMBER PICKER — integrated dropdown */}
    ...
    <div className="absolute z-20 top-full left-0 mt-1 w-full max-w-xs bg-white border border-gray-200 rounded-md shadow-md">
      <ul className="max-h-72 overflow-auto divide-y divide-gray-100">
        ...
        <div style={{ backgroundColor: customColor }} className="rounded-xl p-4 text-white">
          ...
          {/* Selected day -> list projects + leader */}
