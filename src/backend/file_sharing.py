...
def delete_folder(folder_id):
    ...
    for file_doc in files_collection.find({"folder_id": folder_oid}):
        if file_doc.get("storage") == "gridfs":
            try:
...
def delete_file(file_id):
    ...
    if user.get("role") != "admin" and file_doc.get("uploaded_by") != user["_id"]:
        return jsonify({"error": "Permission denied"}), 403
    ...
    socketio.emit(
        "file:deleted",
        {"_id": str(file_oid), "folder_id": str(file_doc["folder_id"])},
        namespace="/rt"
    )
