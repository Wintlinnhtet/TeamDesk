...
def _public_user_doc(u):
    img = u.get("profileImage") or u.get("profileImageUrl") or "cat2.jpg"
    return {
        "id": str(u["_id"]),
        "name": u.get("name", ""),
...
def api_profile():
    ...
    if not u.get("profileImage"):
        users_collection.update_one(
            {"_id": uid},
            {"$set": {"profileImage": "cat2.jpg", "updated_at": datetime.utcnow()}}
        )
        u["profileImage"] = "cat2.jpg"
...
def _abs_image_url(v: str | None) -> str | None:
    """Make a usable absolute URL for any stored image value."""
    if not v:
        return None
    s = str(v).strip()
    if not s:
    ...
    if s.startswith("uploads/"):
        return f"{base}/{s}"
    # anything else: return as-is (public asset/custom path)
    return s
...
def signin():
    ...
    # choose the best image field and normalize to absolute URL
    raw_img = (
        user.get("profileImage")
        or user.get("profileImageUrl")
        or user.get("picture")
        or (user.get("profile") or {}).get("photo")
        or user.get("avatar_url")
        or user.get("avatar")
        or None
    )
    image_url = _abs_image_url(raw_img)

    # optional: a timestamp you can use as cache-buster on the client
    img_updated = user.get("profileImageUpdatedAt") or user.get("updatedAt") or int(time.time())
    ...
    return jsonify({
        ...
        "user": {
            ...
            # 🔥 include image fields the frontend expects
            "profileImage": image_url,
            "profileImageUpdatedAt": img_updated,
        }
    })
# -------------------- USERS (lookup by ids) --------------------
@app.get("/users")
...
def project_detail(project_id):
    ...
    if request.method == "GET":
        ...
        members = [{
            ...
            "name": m.get("name",""),
            "email": m.get("email",""),
            "profileImage": m.get("profileImage") or ""   # ✅ add this line
        } for m in members]
...
def get_user(user_id):
    try:
        ...
        # --- 🔥 ensure profileImage is always included ---
        raw_img = (user.get("profileImage") or "").strip()
        image_url = None
        if raw_img:
            if raw_img.startswith("/uploads/"):
                image_url = f"{request.host_url.rstrip('/')}{raw_img}"
        ...
        return jsonify({
            ...
            "email": user.get("email", ""),
            "experience": exp,
            "profileImage": image_url   # ✅ now included
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
